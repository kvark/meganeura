...
fn main() {
    ...
    let weights_path = std::env::var("V2S_WEIGHTS")
        .unwrap_or_else(|_| "/x/Code/mind-games/ext/efficientnet_v2_s.safetensors".to_string());
    ...
    let (out_label, out_node, out_channels, out_h, out_w): (&'static str, NodeId, u32, u32, u32) =
        match stage.as_str() {
            ...
            "f20_after_bn_e" => (
                "features.2.0 expand AddPerChannel",
                f20_after_bn_e,
                96,
                48,
                48,
            ...
            other => panic!(
                "unknown stage {other:?}; supported: stem_conv|stem_addperch|stem|f10_conv|f10_addperch|f10_silu|f10|f11|f20|f21|f22|f23|f30|f40"
            ),
        };
        ...
        for c in 0..3 {
            for h in 0..H {
                for w in 0..W {
                    ...
                    v[idx] =
                        ((h as f32 / H as f32) + (w as f32 / W as f32) + (c as f32 * 0.3)).sin();
                }
            }
        }
        v
