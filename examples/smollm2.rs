...
fn main() {
    ...
    for (name, _) in session.plan().param_buffers.clone() {
        if name == "lm_head.weight" {
            ...
            // Concatenated weights from SwiGLU fusion. Load raw (untransposed)
            // tensors, concat vertically, then transpose the result if needed.
            ...
            if needs_transpose {
                // combined is [out_total, in] row-major. Transpose to [in, out_total].
