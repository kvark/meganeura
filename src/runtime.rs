...
pub(super) fn wait_for_timed_encoder(
    ...
    assert!(gpu.wait_for(sync, !0)?, "GPU wait timed out");
...
impl Session {
    ...
    fn test_coop_matmul(
        ...
        trace.wait(
            ...
            wait_for_timed_encoder(gpu, &sp, &mut encoder, false).map(|_| true),
        );
    ...
    fn build_session_impl(plan: ExecutionPlan, gpu: Arc<Gpu>, opts: SessionOptions) -> Self {
        ...
        let physical_buffers: Vec<Arc<PhysicalBuffer>> = {
            ...
            alias
                ...
                .map(|(i, &size)| {
                    ...
                    // Keep creation and Shared zeroing interleaved in alias order.
                    // Preserve the explicit parameter-initialization opt-out.
        ...
        if device_zero_allocations != 0 {
            ...
            trace.wait(
                ...
                wait_for_timed_encoder(&gpu, &sp, &mut encoder, opts.gpu_timing).map(|_| true),
            );
        }
