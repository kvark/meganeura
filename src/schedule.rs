...
fn lower_reduction(
    ...
    let prologue_value = if lanes_per_row == 1 {
        // Multiplication by a runtime one prevents a folded producer multiply
        // from contracting with the reduction add. This keeps the fused
        // packed path bit-identical to materializing the producer without a
        // global-memory round trip.
        format!("v{} * bitcast<f32>(params.round_one_bits)", prologue.output)
    } else {
        format!("v{}", prologue.output)
    };
    let _ = writeln!(
        src,
        "            {}",
        op.combine_wgsl("acc", &prologue_value)
    );
