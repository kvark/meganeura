...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Apply RoPE per-head: each head of head_dim elements gets its own rotation.
    let half_head = params.head_dim / 2u;
    let head = pair_in_row / half_head;
    let pair_in_head = pair_in_row % half_head;

    // inv_freq = theta ^ (-2 * pair_in_head / head_dim)
    ...
    // HuggingFace "half-split" convention within each head: pair (d, d + head_dim/2)
