    ...
    fn matches(&mut self) -> Result<bool, TuneError> {
            ...
            .fill_buffer(self.flag.at(0), 4, 0);
        {
            ...
            for image in &self.images {
                ...
                pc.dispatch([len.div_ceil(256).min(65535), 1, 1]);
            }
        }
        self.commands.submit_wait()?;
            ...
            if outcome.qualified {
                ...
                (outcome.baseline_ms, outcome.candidate_ms) =
                    measure_pairs(policy.sample_pairs, |alternative| {
                        ...
                        match self.run(session, commands, count) {
                            Ok(ms) => match self.matches() {
                                Ok(true) if !expired() => Some(ms),
                                Ok(true) => None,
                                Ok(false) => {
...
impl Session {
    ...
    ///
    /// Call after initializing representative inputs, weights and caches, not
    /// during compilation on empty inputs. Trials copy writable allocations,
    /// preserving aliasing and placement; readonly allocations are borrowed.
    /// No live tensor, KV cache or optimizer state is modified. Runtime-appended
    /// optimizer/clip/accumulation passes and output readback are not measured.
    /// No other workload may mutate shared inputs or run competing measurements.
    ///
    /// Complete writable images must match the original schedule bitwise. A
    /// non-repeatable baseline or exhausted bound keeps the incumbent. The
    /// deadline is soft: it is checked between complete probes; setup and GPU
    /// work cannot be preempted. Only the selected submission count is
    /// installed, respecting `max_chunks`; this
    /// explicit call replaces any previous caller-selected chunk policy.
