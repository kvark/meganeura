use super::*;
use crate::tune::{TuneAttention, TuneScope};
use std::sync::Arc;
...
fn collect(session: &Session) -> Vec<Class> {
    ...
    for (first, d) in plan.dispatches.iter().enumerate() {
        ...
        let initial = if combine.is_some() { d.params[6] } else { 1 };
...
impl Pipelines {
    fn ensure_attention(&mut self, gpu: &Gpu, entry: ShaderEntry, dim: u32) -> Result<(), String> {
        let key = Variant::Attention(entry.clone(), dim);
        ...
        let pipeline = gpu.create_compute_pipeline(bg::ComputePipelineDesc {
            name: &key.label(),
            data_layouts: &[&super::super::shader_data_layout(&entry)],
...
impl Session {
        ...
        for member in &class.members {
            ...
            let partial = if splits == 1 {
                ...
                let physical = *shared_physical.get_or_insert_with(|| {
                    ensure_device_memory_budget(&self.gpu, bytes, "attention partials");
    ...
    fn grow_attention_partial(&mut self, partial: BufferRef, bytes: usize) {
        ...
        if bytes > self.alias.sizes[physical] {
            ensure_device_memory_budget(&self.gpu, bytes, "attention partial growth");
            let handle = self.gpu.create_buffer(bg::BufferDesc {
                name: "attention_partials",
                size: bytes as u64,
    ...
    fn measure_attention(
        ...
        let (Some(a), Some(b)) = (key.sizes(outcome.initial), key.sizes(outcome.candidate)) else {
            outcome.decision = TuneDecision::ScratchLimit;
            return;
        };
        let sizes: Vec<_> = a.iter().zip(&b).map(|(&a, &b)| a.max(b)).collect();
        ...
        let variants = [outcome.initial, outcome.candidate]
            ...
            .map(|v| {
                v.iter()
                    .map(|d| {
                        (
                            &self.pipelines.map
                                [&Variant::Attention(d.shader.clone(), key.head_dim)],
                            d,
                        )
                    })
                    .collect()
        ...
        for (i, input) in test_inputs(&sizes[..6], 0).iter().enumerate().take(3) {
        ...
        (outcome.baseline_ms, outcome.candidate_ms) =
            measure_pairs(options.sample_pairs, |alternative| {
                ...
                (start.elapsed() < options.max_time).then_some(
                    elapsed / (key.positions.len() as f64 * options.dispatches_per_sample as f64),
                )
            });
        drop(sampling);
        drop(scratch);
        ...
        for variant in 0..2 {
            ...
            for (case, &(position, valid)) in cases.iter().enumerate() {
                ...
                if !qualify_reference(key, &inputs, &output, position, valid, scale)
                    || (variant != 0 && !outputs_agree(&references[case], &output, scale))
        ...
        for head in 0..key.num_heads {
            ...
            for d in 0..dim {
                let expected = (begin..end)
                    ...
                    .sum::<f64>()
                    / sum;
                if !close(expected, output[q_offset + d], scale) {
                    return false;
                }
            }
        }
...
mod tests {
    ...
    fn attention_contract_and_oracles_cover_geometry_and_corruption() {
        ...
        let mut inputs = test_inputs(&key.sizes(1).unwrap()[..6], 0);
        inputs[0].fill(0.0);
    ...
    fn attention_retuning_preserves_live_output_and_reuses_partial_storage() {
        for capacity in [64, 96] {
            ...
            for splits in [16, 1, 2, 16, 1, 4, 16] {
                ...
                session.step();
                session.wait();
                assert!(outputs_agree(&reference, &session.read_output(24), 1.0));
            }
        }
    }
}
