...
The fourteen campaign archives contain:
...
Outputs are nine LaTeX fragments (including the SmolLM2 chart),
`conditions.csv`, `failures.json`, and `search-summary.json`.
...
From an extracted supplement containing the checker, manifest, stream and tables:
...
**The PDF and ZIPs have not been regenerated by this update.** Earlier
submission bundles describe the old cohort. Rebuild them together at final
packaging; do not combine the new checker with old tables or a v2 record stream.
