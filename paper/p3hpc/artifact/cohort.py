...
PREVIOUS_INFERENA = "fa5a04e1c1b38405cfa371a27c5dcef1319835d5"
PREVIOUS_MEGANEURA = "428fc2d2322229e5338f5d80a10d700340d593cd"
...
DEVICES = {
    ...
    "amd-dgpu": "RX 7900 XT",
    "amd-780m": "Radeon 780M",
    "intel-b570": "Arc B570",
    "apple-m3": "Apple M3",
    "intel-igpu-qualify": "Intel RPL-U",
    "amd-igpu-qualify": "Ryzen 9600X iGPU",
}
...
def audit_search(session, legacy):
    search = session["search"]
    if legacy:
        require(search["scope"] == "All" and search["class_limit"] is None
                and not search["class_limit_reached"] and search["max_seconds"] == 60.0
                and search["max_scratch_bytes"] == 1024**3, "legacy search policy changed")
        require(0 <= search["visited_classes"] <= search["eligible_classes"]
                ...
                "unexplained legacy search truncation")
        require(sum(search["decisions"].values()) == search["comparisons"], "legacy decision count differs")
        return {"elapsed_seconds": search["elapsed_seconds"], "graphs": 1, "programs": 1,
                "truncated": search["time_budget_exhausted"], "extraction_truncated": False,
                "visited_classes": search["visited_classes"], "eligible_classes": search["eligible_classes"],
                "decisions": search["decisions"], "program_rejections": 0, "selected_graph": 0,
                "selected_description": "legacy kernel-only search", "phase_seconds": {}}
    options, trials = search["options"], search["trials"]
    tuning, memory = options["tuning"], session["memory_budget"]
    require(seconds(options["max_time"]) == 60 and options["max_graphs"] == 4
    ...
    require(tuning["scope"] == "All" and tuning["max_classes"] == 2**64 - 1
            and seconds(tuning["max_time"]) == 2 and tuning["max_scratch_bytes"] == 1024**3,
    ...
    for trial in trials:
        ...
        require(k["scope"] == "All" and k["max_classes"] == tuning["max_classes"]
                and k["max_scratch_bytes"] == 1024**3 and 0 <= seconds(k["max_time"]) <= 2
...
def audit_pair(pair, campaign, run):
    ...
    for engine, record in pair.items():
        ...
        require(record["protocol"]["measurement_runs"] == count, "sample count changed")
    ...
    legacy = campaign["source"] == PREVIOUS_INFERENA
    require(mg["optimizer"].get("measured_kernel_search" if legacy else "measured_construction") is True,
    ...
    mode = "default" if campaign["args"]["collect"] else "eager"
    require(execution["requested_mode"] == run["mode"] == mode
    ...
    require(errors["output_relative_l2_error"] < 0.01 and errors["loss_relative_error"] < 0.01,
            "forward gate failed")
    limit = 0.10 if mg["precision"]["reduced_precision_allowed"] and campaign["args"]["collect"] else 0.05
    ...
    with tarfile.open(path, mode="r|gz") as archive:
        for member in archive:
            if member.isfile() and member.name.endswith(".json"):
                yield member.name, json.load(archive.extractfile(member))


def read_records(entries, legacy, bundle=None):
        ...
        for record in value if isinstance(value, list) else [value]:
            ...
            if record["framework"] == "meganeura" and record["status"] == "ok":
                for session in record["optimizer"]["sessions"]:
                    session["search"] = audit_search(session, legacy)
        records[name] = value
    return records


def load_campaign(path, records, legacy=False):
    ...
    require(campaign["source"] == (PREVIOUS_INFERENA if legacy else INFERENA)
            and campaign["meganeura"]["rev"] == (PREVIOUS_MEGANEURA if legacy else MEGANEURA),
            "campaign revision mismatch")
    require(campaign["torch"]["git_version"] == TORCH, "campaign PyTorch mismatch")
    require(campaign["protocol"] == ("p3hpc-paired-campaign-v9" if legacy else "p3hpc-paired-campaign-v10"),
    ...
    policy.update({"measured_kernel_search": True} if legacy else {
        "measured_construction": True, "optimizer": "egglog-outlined", "kernel_seconds_per_program": 2.0,
...
def complete_devices(campaigns, rows, precision):
    return [device for device, c in campaigns.items() if c["args"]["backend"] != "cpu"
            and all(rows[device].get((precision, model, primary_condition(c)), {}).get("replicates") == 3
                    for model in MODELS)]
...
def tables(campaigns, rows, groups, previous):
    output = {}
    ...
    for device, label in paired_devices.items():
        ...
        lines.append([label, backend.upper() + "/compiled", driver, replay, f"{valid}/{expected}", c["status"]])
    output["devices.tex"] = tex_table("llllrl", "Device & PyTorch path & Graphics driver & Explicit replay & Pairs & Campaign", lines)
    lines = []
    for device, label in DEVICES.items():
        if device in paired_devices:
            continue
        conditions = len(groups[device])
        stage = "XPU device unavailable" if device == "intel-igpu-qualify" else "ROCm crash / incorrect output"
        lines.append([label, f"Vulkan: {conditions}/10", "No usable GPU path", stage])
    ...
    lines = []
    for model, device in (("SmolLM2-135M", "nvidia-h100"), ("SmolLM2-360M", "nvidia-h100-large"), ("SmolLM2-1.7B", "nvidia-h100-large")):
        row = previous[device]["strict", model, "default-graph1"]
    ...
    for model in MODELS:
        ...
        for phase in PHASES:
            ratios = [rows[device]["strict", model, primary_condition(c)]["ratio_" + phase]
                      for device in complete_devices(campaigns, rows, "strict")
                      for c in [campaigns[device]]]
...
def main():
    ...
    parser.add_argument("--previous", type=Path, help="earlier cohort directory; kept separate from the current results")
    ...
    campaigns, rows, all_groups, previous, failures = {}, {}, {}, {}, {}
    digests = {}
    for line in (HERE / "cohort.sha256").read_text().splitlines():
        expected, name = line.split("  ", 1)
        require(len(PurePosixPath(name).parts) == 2 and PurePosixPath(name).parts[0] in ("current", "previous"),
                "unexpected manifest path")
        digests[name] = expected
        if args.archives.is_dir():
            root = args.previous if name.startswith("previous/") else args.archives
            require(root is not None, "supply --previous for the separately labeled earlier cohort")
            with (root / PurePosixPath(name).name).open("rb") as file:
    ...
    with (nullcontext(None) if args.archives.is_dir() else lzma.open(args.archives, "rt")) as source, \
        ...
        if source:
            require(json.loads(next(source)) == {"format": "p3hpc-json-files-v2", "archives": digests}, "bundle provenance differs")
        if bundle:
            bundle.write(json.dumps({"format": "p3hpc-json-files-v2", "archives": digests}) + "\n")
        for name in digests:
            legacy = name.startswith("previous/")
            path = (args.previous if legacy and args.archives.is_dir() else args.archives) / PurePosixPath(name).name
            if source:
            ...
            records = read_records(entries, legacy, bundle)
            if bundle:
                bundle.write("null\n")
            c, groups, failed = load_campaign(path, records, legacy)
            if legacy:
                require(c["status"] == "complete" and not failed, "incomplete earlier campaign")
            for name, digest in c["sha256"].items():
                name = name.replace("\\", "/")
                require(input_hashes[legacy].setdefault(name, digest) == digest, "input identity differs: " + name)
            device = "amd-780m" if legacy and path.stem == "amd-igpu" else path.stem
            if legacy:
                previous[device] = aggregate(groups)
            else:
            ...
            print("previous" if legacy else "current", device, c["status"],
                  dict(Counter(run["status"] for run in c["runs"])), flush=True)
            del records
        if source:
            require(not source.read().strip(), "unexpected trailing campaign")
    for name in input_hashes[False].keys() & input_hashes[True].keys() - {"Cargo.lock"}:
        require(input_hashes[False][name] == input_hashes[True][name], "checkpoint changed across cohorts: " + name)
    generated = tables(campaigns, rows, all_groups, previous)
    if args.output:
        args.output.mkdir(parents=True, exist_ok=True)
        for name, content in generated.items():
            (args.output / name).write_text(content)
        flat = [{"device": device, "precision": key[0], "model": key[1], "condition": key[2],
                 "fully_replicated": row["replicates"] == 3, **row}
        ...
        (args.output / "previous-conditions.json").write_text(json.dumps({device: [
            {"precision": key[0], "model": key[1], "condition": key[2], **row}
            for key, row in sorted(data.items())] for device, data in previous.items()}, indent=2) + "\n")
    if args.check:
        for name, content in generated.items():
    ...
    for precision in ("strict", "accelerated"):
        ...
        for phase in PHASES:
            ...
            old = [previous[device][precision, model, primary_condition(campaigns[device])]["ratio_" + phase]
                   for device in devices for model in MODELS]
            print("  previous cohort, SAME devices/workloads: median ratio", statistics.median(old))
