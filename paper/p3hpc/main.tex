...
Its compilation step measures alternative graph and kernel implementations.
...
The seven-platform collection retains 193 numerically valid process pairs;
two AMD campaigns stop early. Across six fully replicated strict
32-bit floating-point configurations, median inference and training times
are $1.99\times$ and $2.29\times$ PyTorch's.
...
Compilation can measure alternatives before choosing a graph and its
kernels. This search is optional in the library and enabled in every
benchmark run.
...
Egglog retains equivalent expressions using bounded equality saturation
\cite{zhang2023egglog}. Large graphs are divided into smaller regions;
repeated regions reuse the same optimization work. Training adds reverse-mode
...
have been timed. Both paths use the same rewrite rules and lowering code.
...
Graph forms therefore compete with their tuned implementations, rather than
being discarded before tuning begins.

The benchmark allows four graph forms and 64 programs per session, with a
...
All 193 completed GPU pairs and 20 CPU-oracle qualification pairs
...
RTX~5070, H100, RTX~3050, Arc~B570, and Apple~M3 complete all 30 pairs.
...
is complete. RX~7900~XT retains 18 pairs before a manual interrupt in
the second accelerated ResNet run; the log does not establish a framework
crash or timeout. Neither campaign was retried.
...
The current total is 193 valid GPU pairs out of 210 planned. One further
pair fails and one is interrupted; 15 are not reached. The two graphics-only
...
Bars are medians of three process medians, except RX~7900~XT ($n=2$).
...
Across the six fully replicated strict configurations, median ratios are
1.99 for inference, 1.58 for minimal shapes, and 2.29 for training.
Meganeura is nominally faster in 5/30, 10/30, and 2/30 comparisons.
The accelerated aggregate covers five configurations, excluding both
partial AMD sweeps: ratios are 2.75, 1.75, and 3.26, with 1/25, 4/25,
and 0/25 nominal wins. Different platform sets make these aggregates
...
is also faster. The partial RX~7900~XT data show SmolLM2 and SmolVLA
minimal-shape ratios of 0.19 and 0.22, and SmolVLA training at 0.61.
These two-process observations are useful, but not a completed sweep.
...
Over the same six strict platforms, the training ratio improves from
2.58 to 2.29, while inference changes from 1.94 to 1.99.
...
Table~\ref{tab:search} counts search work in the 193 completed GPU pairs,
including partial campaigns. Their 541 sessions try 8,832 programs.
In 83 sessions the selected graph differs from ordinary extraction.
Another 167 sessions report unfinished program search; the longest takes
66.12\,s against the soft 60-second deadline.
Extraction separately reports truncation in 234 sessions. In 117 sessions
...
Across program trials, private probes visit 122,352 of 171,875 eligible
kernel-class instances. Repeated classes and reused comparisons mean
these counts are not distinct algorithms. There are 12,082 accepted
kernel replacements, 1,240 kernel-output rejections, and 86 whole-program
...
\begin{table}[t]
\caption{Strict scores over six fully replicated GPU configurations.}
\label{tab:pp}
\centering
\small
\setlength{\tabcolsep}{4pt}
...
policies. Table~\ref{tab:pp} uses the same six GPU-reference systems for
...
Meganeura/PyTorch means are 0.47/0.93 for inference, 0.58/0.91 for minimal
shapes, and 0.38/0.99 for training.

The set excludes the incomplete RX~7900~XT sweep, both CPU-oracle
qualifications, the earlier H100 size study, and MI300X. It is a performance
...
The current compiler explores more graph and kernel choices, but each
program still has only two seconds for private probes. Visiting the
expensive classes early remains important. Small-output,
...
Table~\ref{tab:preparation} sums the 193 completed process pairs, with
...
Across the six fully replicated strict configurations, process-median
preparation spans 19.38--188.45\,s native and 1.88--84.18\,s PyTorch.
The medians over these 30 workload/device groups are 105.33 and 31.24\,s.
...
Across completed GPU pairs, private kernel search accounts for 95.43
cumulative minutes, candidate initialization for 57.40, and full-program
qualification callbacks for 64.35. Much of this work is allocation,
...
the Vulkan path does not need those overrides. The RX~7900~XT interrupt
has insufficient evidence for the same attribution. Windows, meanwhile,
...
separate. Strict aggregates cover six complete configurations;
accelerated aggregates cover five. Partial AMD observations are shown
...
comparison. All 193 completed GPU pairs and 20 qualification pairs
...
Graphics APIs provide a useful ML deployment path. Meganeura executes
inference and backward workloads on consumer GPUs and H100, including
Windows, and qualifies two integrated GPUs without a usable PyTorch GPU
path. The current collection retains 193 valid GPU pairs and reports its
two interrupted campaigns rather than hiding them.

PyTorch still leads most timing comparisons. Across six fully replicated
strict configurations, median inference and training times are $1.99\times$
and $2.29\times$ PyTorch's. Broader graph and kernel search improves several
workloads, but also exposes regressions and substantial preparation cost.
The earlier 1.7B study extends the evidence to larger models; it does not
establish the new optimizer's scaling.

Combining graph compilation and measured choice in one small runtime is
promising. The next step is to make those choices cheaper and more reliable,
while closing concrete kernel and driver gaps. Hardware availability belongs
in that assessment just as much as speed: Vulkan reaches devices that the
tested PyTorch stack does not, while MI300X shows where our own path stops.
...
measurement campaigns, two incomplete, and two CPU-oracle qualifications.
...
Original archive SHA-256 digests identify provenance. The two supplied
AMD failure/interrupt logs and the final runner logs are included separately;
