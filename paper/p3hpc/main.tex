...
are now implementation alternatives in the same e-graph. This covers
...
The primary tables measure the earlier v10 protocol, before these schedule
equalities and broader regions. It allows four graph forms and 64 programs per session, with a
...
The next-cohort policy retains sixteen graph/implementation forms, interleaves
graph and physical-plan choices, and shares the remaining session deadline
with private probes. Paired whole-program comparisons require two warmup
pairs and 250\,ms, subject to that deadline. These changes are not results
of the frozen cohort.
...
separate three checks in \system{}'s development suite. This suite was
added after the cohort reported below; it is not a retrospective certificate
for those binaries.\footnote{\href{https://github.com/kvark/meganeura/tree/c475fd0e05f72929eca3bf994584805596b80b8c/src/reference}{Reference interpreter and checks at revision \code{c475fd0}}.}
...
cross-engine gates (Section~\ref{sec:gates}). Its follow-up collector
...
selection in the timed cohort. The separate ROCm Whisper investigation in
that section uses a different, explicitly recorded attention policy.
...
In the reported cohort, capture or validation failures abort collection;
no eager timing replaces a failed graph run. The later phase-level failure
policy in Section~\ref{sec:validation} does not change these archived results.
...
The earlier rewrite-only ablation found essentially the same active graphs
with greedy and outlined egglog, and no GPU gain distinguishable from
run-to-run variation. SmolLM inference rewriting took 0.089\,ms with greedy,
2.94\,ms with outlining, and 56.2\,ms with whole-graph saturation.
Egglog retains alternatives through measurement; it does not make the same
kernel faster.

Table~\ref{tab:tile-study} tests that expanded search at development revision
\code{2153aeb}, whose tree was merged as \code{e8d7d9e}. Both arms use egglog;
one extracts once and runs private kernel probes, the other retains tile
and split-K alternatives for whole-step comparison. Strict f32 products,
with cooperative matrices disabled, pass a full-output f64 oracle.
Each entry is the upper median of eight profiled GPU samples after twelve warmup
steps, from one process per GPU. These are kernel diagnostics, not cohort
wall times or a controlled greedy-versus-egglog comparison.

\begin{table}[t]
\caption{GPU time in $\mu$s: one graph + tuner $\rightarrow$ measured
schedule equalities. Dimensions are $M\times N\times K$.}
...
The wider search is 1.27--1.66$\times$ faster on NVIDIA and
1.22--1.63$\times$ on Intel. Preparation is not held equal: per-product
construction, qualification and brief readout rise from 0.4--0.7 to
6.3--7.1\,s on NVIDIA and from 0.8--1.7 to 7.4--12.5\,s on Intel.
Private probes use the same policy: two seconds per program, while measured
construction permits forty seconds and 24 programs. A preceding split-K enumerator
reached 19--21\,$\mu$s on the shorter NVIDIA products. The benefit is
finding the schedule; egglog supplies a shared way to retain it alongside
graph rewrites. This does not establish an advantage over an equally
capable enumerator or predict whole-model gains.
...
Earlier max-autotune findings are separate from these current records.\footnote{\href{https://github.com/kvark/inferena/blob/06f2f800a4a57254d25b3331f1876631305a15ab/EXPERIMENT.md}{Inferena protocol and bring-up notes, revision \code{06f2f800}}.}

A separate RX~7900~XT study finds a numerical portability failure in
default-compiled PyTorch 2.13.0/ROCm 7.2. Switching to deterministic uniform
weights (\path{name-index-uniform-v1}) exposed Whisper training drift
already observed with sinusoidal weights. Two uncaptured strict-f32 calls
differ by up to $1.97\times10^{-3}$, exceeding the $9.10\times10^{-6}$
bound under unchanged tolerances before HIP capture; inference capture passes.
With math SDPA, the fully eager encoder repeats but the compiled encoder
does not, even when SDPA itself is eager. Eager efficient SDPA gives bit-exact
training outputs in an initial control, but a later run with that policy also
fails at the same check, with maximum error $2.35\times10^{-3}$.
It is not a qualified workaround, and the passing control does not
isolate the fault to math SDPA.\footnote{\href{https://github.com/kvark/inferena/blob/757f6a80b3dee2d6e9392fbbfc28e64febc9fe28/EXPERIMENT.md}{Initial ROCm Whisper controls and reference policy, revision \code{757f6a8}}.}
This is unresolved numerical portability in the compiled PyTorch/ROCm path,
not a setup problem. Retrying until a process passes would select a successful
run, not repair the failure. These diagnostics remain separate from the timing
cohort; no tolerance is relaxed or eager timing substituted.
...
The schedule study in Section~\ref{sec:egraph-ablation} motivates searching
implementations jointly with graph forms. Logical tensor bytes alone cannot
rank their execution time. Reusing measured kernel-class costs could improve
...
still worth doing. The next cohort must check whether the expanded schedule
search improves whole-model inference at a cost we can afford, and close
...
The \href{https://github.com/kvark/meganeura/blob/e8d7d9e3b2192671c0a9fb1aab2e3fc505153260/examples/search_study.rs}{schedule diagnostic}
and timeline/compiler experiments retain separate source refs and do not enter
cohort aggregates. The \href{https://github.com/kvark/dinovision/tree/dc35cdf1c7c910cdd93c5b5362846842ae469a21}{DinoVision source}
