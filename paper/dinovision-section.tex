...
resizes an RGB frame to $224\times224$, executes the first three layers of a
frozen DINOv3 ViT-S/16 encoder, rearranges its
...
\system{}'s NCHW-flat per-channel multiplication was applied directly to a
$[\mathrm{tokens},\mathrm{hidden}]$ LayerScale branch. Only the first token
received the intended residual update, even though output images remained
plausible. We rejected all weights and measurements from that graph, corrected
the layout, added exact attention and LayerScale CPU tests, and retrained all
replicates.
...
The failure suggests a reusable API hardening change: graph construction
should validate the flattened element count, channel/spatial product, and gate
length of per-channel multiplication. The compiler revision used here derives
dispatch length from the first shape coordinate, so an inconsistent layout
...
\url{https://github.com/kvark/dinovision}.
