...
|---|---|---|
| Addressed: convolution reciprocal indexing | Full GPU oracle reproduced wrong dW at a width-41 batch boundary. Shared scalar and generated cooperative indexing now uses integer division; four reciprocal uniforms and the tuner-only f32 filter are removed. Full forward/dX/dW regressions pass at 41/47/55 with unchanged numerical gates. | [Cost check](experiments/conv-indexing-2026-09-06/README.md) retains full-state identity but observes ResNet about 23% slower. Recover cost with exact indexing; native-f32 cooperative execution and cross-backend qualification remain due. Never restore approximate addressing for speed. |
