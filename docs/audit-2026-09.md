...
|---|---|---|
| P1: convolution reciprocal indexing outside the admitted search domain | CPU f32 arithmetic gives `u32(f32(41) * (1.0_f32/41.0)) == 0`, not 1. The new tuner rejects such decompositions, but ordinary compilation still uses the existing templates without that guard. This is a source/CPU counterexample, not a retained GPU error-rate experiment. | Exact or corrected integer decomposition in the reusable convolution kernels, adversarial full forward/dX/dW GPU oracles, then a separately declared performance check. Do not widen eligibility by weakening the proof. |
