...
|---|---|---|
| P1: new tuner needs fleet qualification | Scalar GPU tests pass; native-f32 search has offline shader/geometry/padding coverage only. | Native-f32 device tests, whole-step holdouts and repeated-process/fleet evidence before default-on. |
...
The existing CI audit-regression job now also runs the memory, Adam/LaProp,
accumulation and clipping suites alongside checkpoint tests. Local RTX checks
do not imply that the hosted Metal/software-Vulkan jobs have already run.
...
qualify the new native-f32 candidates on supporting hardware, extend the
whole-step transfer pilot to holdouts, and qualify logical checkpoint restores
across backends. The scalar search and lazy-memory/checkpoint changes have
passed initial RTX qualification; broader holdouts should precede
profile-motivated convolution/attention changes. Allocation failures and peak
memory remain separate engineering work, not solved by preflight validation.
