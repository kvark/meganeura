...
| P1: dependency/release qualification | Development now uses published Blade 0.9, blade-macros 0.3 and Naga 30; Rust minimum rises to 1.92 and full package verification is restored in CI. Historical git-pin packaging gap is removed. | Verify/publish the reviewed Meganeura release; maintain consumer, MSRV, advisory and cross-backend qualification. |
...
ResNet's new phase timers put 98.26–98.65% of search wall time in qualification:
median 538 ms out of 546 ms total, versus 7 ms sampling. Split transfer/readback,
CPU checking and device time next; the staging memory class is a source-based
hypothesis, not an established bottleneck. Keep numerical gates and frozen
results unchanged. Telemetry records substantial clock/temperature changes
...
qualify the new native-f32 candidates on supporting hardware, use the retained
crossover/phase evidence to investigate qualification and transfer costs, and
