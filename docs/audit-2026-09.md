...
|---|---|---|
| P1: new tuner needs transfer/fleet qualification | Scalar tests and all 30 holdout state checks pass, but no holdout whole-step gain clears the guard. Native-f32 has offline coverage only. | A/A and crossover confirmation, native-f32 hardware and fleet evidence before default-on. |
...
new timers. A new phase profile is needed to localize the old cost hypothesis.
...
qualify the new native-f32 candidates on supporting hardware, add A/A/crossover
controls and phase profiles after the first holdouts, and qualify logical
checkpoint restores across backends. The scalar search and lazy-memory/checkpoint
changes have passed initial RTX qualification; the holdouts establish state
parity but no guard-passing gain. Use profiles to motivate reusable convolution/
