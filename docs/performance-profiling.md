...
Blade writes at most 1,000 timestamps per submission and silently skips the
rest, so no single replay can measure a larger plan — and a decode or training
step of a real model runs well past that.
...
Two numbers shift meaning once `window_count` exceeds 1.
`profiled_wall_samples_ms` holds one entry per replay rather than per sample,
and `instrumentation_wall_ratio` understates full-instrumentation overhead,
because each replay runs most of the plan at nearly normal pass counts.
`gpu_total_samples_ms` is unaffected: it sums a sample's windows, so it still
