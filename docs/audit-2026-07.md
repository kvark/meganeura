...
| Optimizer strategy | Strong and differentiated | Traffic-aware extraction owns rewriting, stamps selected terms per repeated-region instance, and segments the rest of the graph for saturation. |
...
- **Extraction/stamping is newly load-bearing.** Equality saturation now owns
  rewriting: selected terms are stamped per repeated-region instance and the
  remainder of the graph is segmented for saturation. This resolves the former
  split with global pattern appliers. Cross-segment fusion and continued
  stress-testing of node-id-preserving stamping remain important.
...
research direction. Extracted terms now directly rewrite the IR, repeated
regions are stamped per instance, and non-region nodes are segmented so the
whole graph passes through the e-graph. The former hand-written global appliers
and constant-cost escape hatch are gone. The repository also keeps
rejected-optimization notes and wall-clock guidance, which is unusually
healthy for a performance project.

The remaining risk has shifted from split decision ownership to the correctness
of a newly central stamping mechanism. Segment boundaries are opaque to
cross-segment fusion, and optimizer reports should eventually explain the
selected alternative and estimated bytes saved per region. Node-id,
id-carrying-attribute, and topological-order invariants need continued focused
tests as more rewrite constructors are added.
...
- The final tree exposes 350 tests: 346 execute successfully and four are
