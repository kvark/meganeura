//! Numerical correctness for the *full* LLM temporal decoder
//! (`build_temporal_decoder`): per-frame RVQ token grid → shared token embed →
//! mean-pool over the levels → N temporal decoder layers (causal self-attn +
//! cross-attn to the encoder) → per-frame temporal states `[num_frames, embed]`.
//!
//! Built with small random weights and a zero (shared) rel-pos table — so the
//! self-attention is plain causal — run on the GPU, and compared against an
//! independent CPU reference. Extends `llm_decoder_stack_correctness.rs` by
//! covering the real temporal *input* construction: the mean over a frame's
//! `num_levels` level embeddings (`depthformer.py`'s `MultivariateDecoder`).

use std::collections::HashMap;

use meganeura::models::magenta_rt::llm::{build_temporal_decoder, LlmConfig};
use meganeura::Graph;
...
#[test]
fn temporal_decoder_matches_cpu_reference() {
    let cfg = tiny_cfg();
    let embed = cfg.embed_dim as usize;
    let vocab = cfg.vocab_size as usize;
    let levels = cfg.num_levels as usize;
    let grid = FRAMES * levels; // flattened decoder input length
    let enc_seq = cfg.encoder_seq_len as usize;

    let mut r = Rng(0x5EED_1234_BEEF);
    ...
    // One token per (frame, level): FRAMES*LEVELS ids in [0, vocab).
    let tokens: Vec<u32> = (0..grid).map(|i| ((i * 7 + 3) % vocab) as u32).collect();
    let enc = r.vec(enc_seq * embed);

    // GPU.
    let mut g = Graph::new();
    let tok_node = g.input_u32("dec_tokens", &[grid]);
    let enc_node = g.input("enc_out", &[enc_seq, embed]);
    let states = build_temporal_decoder(&mut g, &cfg, tok_node, enc_node, FRAMES);
    ...
    // Single shared rel-pos table for the whole temporal sub-decoder (zeroed).
    s.set_parameter(
    ...
    s.step();
    s.wait();
    let gpu = s.read_output(FRAMES * embed);

    // CPU reference: embed the grid, mean-pool each frame's `levels` embeddings,
    // then run the temporal layers.
    ...
    assert_eq!(gpu.len(), x.len());
    let mut max_abs = 0.0_f32;
    for (i, (&a, &b)) in gpu.iter().zip(x.iter()).enumerate() {
        let d = (a - b).abs();
    ...
    eprintln!("temporal decoder max abs diff GPU vs CPU: {max_abs:.2e}");
}
