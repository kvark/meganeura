...
fn record(seed: usize) -> Value {
    archives::json(
        &std::fs::read(format!(
            "{}/docs/experiments/split-k-sequence-2026-09-06/run-{seed:02}.json.gz",
...
fn bounded_accuracy_replay_keeps_rejection_distinct_from_test_process_success() {
    let original = archives::json(include_bytes!(
        "../docs/experiments/compensated-dw-2026-09-06/accuracy.json.gz"
    ));
