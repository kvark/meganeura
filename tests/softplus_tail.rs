...
fn softplus_tail_forward_and_backward_match_f64() {
    ...
    for beta in [0.25f32, 1.0, 10.0] {
        for schedule in [false, true] {
            ...
            let upstream: Vec<_> = (0..input.len())
            ...
            let options = CompileOptions {
                use_schedule_pointwise: schedule,
                ..Default::default()
            };
...
fn softplus_tail_normalized_mixture_matches_f64() {
    ...
    .0;
    s.set_input("x", &logits);
    s.set_input("prior", &priors);
    s.step();
    s.wait();
    ...
    for row in 0..2 {
        ...
        for (c, &weight) in raw.iter().enumerate() {
            accurate(output[row * 6 + c], weight / total, "normalized weight");
        }
    }
}
