...
fn portable_checkpoint_roundtrips_between_direct_and_shared_winograd() {
    ...
    for i in 0..2 {
        let mut a = vec![0.0; 64 * 8 * 8];
        let mut b = a.clone();
        direct.read_output_by_index(i, &mut a);
        optimized.read_output_by_index(i, &mut b);
        let error = a
            .iter()
            .zip(b)
            .map(|(a, b)| (a - b).abs())
    ...
    let mut restored = vec![0.0; weights.len()];
    direct.read_param("kernel:winograd", &mut restored);
