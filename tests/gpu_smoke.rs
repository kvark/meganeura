...
fn lr_multipliers_apply_to_sgd_update() {
    ...
    let delta_a_default: f32 = a_after.iter().zip(init.iter()).map(|(p, i)| (p - i).abs()).sum();
    let delta_b_default: f32 = b_after.iter().zip(init.iter()).map(|(p, i)| (p - i).abs()).sum();
    ...
    let delta_a_scaled: f32 = a_after2.iter().zip(init.iter()).map(|(p, i)| (p - i).abs()).sum();
    let delta_b_scaled: f32 = b_after2.iter().zip(init.iter()).map(|(p, i)| (p - i).abs()).sum();
    ...
    let delta_b_cleared: f32 = b_after3.iter().zip(init.iter()).map(|(p, i)| (p - i).abs()).sum();
    assert!(
