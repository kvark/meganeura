...
fn fused_rms_norm_add_matches_cpu_for_prefill_and_decode() {
    ...
    // A pointwise consumer can be folded into the Add before this fusion
    // runs. The RmsNormAdd shader has no general pointwise epilogue, so the
    // fusion must leave that dispatch alone rather than lose the scale.
    let (rows, cols) = (4, 256);
    let input = (0..rows * cols)
        .map(|index| ((index * 17 % 101) as f32 - 50.0) / 23.0)
        .collect::<Vec<_>>();
    ...
    let residual_node = graph.input("residual", &[rows, cols]);
    let normalized = graph.rms_norm(input_node, weight_node, EPS);
    let added = graph.add(normalized, residual_node);
    let output = graph.scale(added, std::f32::consts::FRAC_1_SQRT_2);
    graph.set_outputs(vec![output]);
    ...
    assert!(
        session
            ...
            .dispatches
            .iter()
            .all(|dispatch| dispatch.shader != meganeura::compile::ShaderEntry::RmsNormAdd),
        "RmsNormAdd cannot preserve an Add's pointwise epilogue"
    );
    session.set_input("input", &input);
    session.set_parameter("weight", &weight);
    session.set_input("residual", &residual);
