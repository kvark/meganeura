...
fn cache_writes_alias_before_following_views_are_allocated() {
    for prefix in [false, true] {
        ...
        let input = graph.input("new", &[1, 8]);
        let cache = graph.parameter("cache", &[4, 8]);
        let position = graph.input_u32("position", &[1]);
        let written = if prefix {
            let valid = graph.input_u32("valid", &[1]);
            graph.cache_write_prefix(input, cache, position, valid)
        } else {
            graph.cache_write(input, cache, position)
        };
        let view = graph.reshape(written, &[8, 4]);
        let view = graph.reshape(view, &[32]);
        graph.set_outputs(vec![view]);
